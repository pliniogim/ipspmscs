...
@section('conteudo')
  ...
  <form method="POST" action="{{action('AtivoController@update', $id)}}">
    ...
    <div class="form-group">
      ...
      <input type="text" value="{{ $id->telefone }}" placeholder="(xx)xxxx-xxxx" pattern="[\(]\d{2}[\)]\d{4}[\-]\d{4}" maxlength="40" class="form-control"  id="telefone"  name="telefone" oninvalid="this.setCustomValidity('Por favor, preencha este campo.')"
      oninput="setCustomValidity('')" required>
    </div>
